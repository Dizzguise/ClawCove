...
![Clawville screenshot](https://raw.githubusercontent.com/your-org/clawville/main/docs/screenshot.png)
...
```bash
# Clone and install
git clone https://github.com/your-org/clawville
cd clawville
npm install
...
```
Browser → ws://127.0.0.1:2788/ws → Clawville server → ws://127.0.0.1:18789 (OpenClaw)
```

Clawville runs a tiny local server that proxies your gateway WebSocket. The browser never touches the gateway directly — the server handles auth, discovery, and event streaming. Zero CORS issues, token never exposed to the browser.

On first run, Clawville queries your gateway for:
...
The resulting city layout is saved to `~/.openclaw/workspace/clawville/layout.json`. On subsequent runs, building positions are preserved, new buildings appear, and removed buildings slowly sink into the ocean floor.
...
```bash
# Different port
CLAWVILLE_PORT=3000 npm start

# Don't auto-open browser
CLAWVILLE_NO_OPEN=1 npm start
...
Your gateway has auth configured but Clawville couldn't find the token. Make sure `gateway.auth.token` is set in your `~/.openclaw/openclaw.json`. Clawville reads it automatically.

**City looks wrong / missing buildings**
Delete `~/.openclaw/workspace/clawville/layout.json` to regenerate from scratch.

---

## What Clawville does NOT do
...
Clawville is a completely independent project. It connects to OpenClaw the same way the CLI or Control UI does — as a WebSocket client. No PRs, no forks, no changes to OpenClaw required.
