...
export function generateWorld(state) {
  ...
  agentDefs.forEach((agDef, i) => {
    ...
    // Prefer coded names/ids for stable representation
    const buildingName = isManager ? `${agId} Command`
      : hierarchyRole === 'specialist' ? `${agId} Lab`
      : `${agId} Reef`;
    const buildingLabel = isManager ? `${agId.toUpperCase()} COMMAND`
      : hierarchyRole === 'specialist' ? `${agId.toUpperCase()} LAB`
      : `${agId.toUpperCase()} REEF`;
