...
export function generateWorld(state) {
  ...
  // Hierarchy roles:
  //   manager    = main agent (orchestrator / business manager)
  //   specialist = named sub-agents with known functional roles
  //   employee   = all other sub-agents
  const SPECIALIST_IDS = new Set(['browser','code','canvas','voice','whatsapp','discord','telegram','slack']);
  ...
  agentDefs.forEach((agDef, i) => {
    ...
    const hierarchyRole = isMain ? 'manager'
      : SPECIALIST_IDS.has(agId) ? 'specialist'
      : 'employee';

    // Manager gets a unique central-north position, not in the rotation pool
    const effectiveZone = isMain
      ? { cx: 20, cy: 14 }   // always NW anchor for manager
      : zone;
    ...
    const buildingName = isMain ? "Manager's Grotto"
      : hierarchyRole === 'specialist' ? `${capitalize(agId)} Lab`
      : `${capitalize(agId)}'s Reef`;
    const buildingLabel = isMain ? 'MANAGER GROTTO'
    ...
    buildings.push({
      ...
      // Manager has a flag/crown marker, specialist has tools marker
      marker: isMain ? 'crown' : hierarchyRole === 'specialist' ? 'tools' : null,
      details: {
      ...
      activities: isMain
        ? ['Delegating task…','Reviewing output…','Planning strategy…','Briefing team…']
    ...
    // Sprite name: manager is always Molty, specialists keep their id, employees get names
    const EMPLOYEE_NAMES = ['Wally','Digby','Byte','Coral','Finn','Dex','Nemo','Patch'];
    const spriteName = isMain ? 'Molty'
      : hierarchyRole === 'specialist' ? capitalize(agId)
      : EMPLOYEE_NAMES[i % EMPLOYEE_NAMES.length];

    agents.push({
      id: agId,
      name: spriteName,
      role: hierarchyRole === 'manager' ? 'Manager'
          : hierarchyRole === 'specialist' ? `${capitalize(agId)} Specialist`
          : 'Employee',
      hierarchyRole,
