// lib/worldgen.js
// Converts OpenClaw gateway state into a ClawCove city layout.
...
export function generateWorld(state) {
  ...
  const agentDefs = normalizeAgentDefs(cfg?.agents);
  const managerAgentId = detectManagerAgentId(agentDefs);
  ...
  // Hierarchy roles:
  //   manager    = configured default/orchestrator agent
  //   specialist = known functional operators (browser/channel/tool operators)
  //   employee   = all other workers
  const SPECIALIST_IDS = new Set(['browser','code','canvas','voice','whatsapp','discord','telegram','slack','web-scout']);

  agentDefs.forEach((agDef, i) => {
    const agId = agDef.id ?? `agent-${i}`;
    const displayName = resolveAgentDisplayName(agDef, agId);
    const model = normalizeModelRef(agDef.model ?? cfg?.agents?.defaults?.model ?? 'openai-codex/gpt-5.3-codex');
    const tier = modelTier(model);
    const size = TIER_SIZE[tier];
    const color = agentColor(agId);
    const zone = agentZones[i % agentZones.length];
    const isManager = agId === managerAgentId;

    const hierarchyRole = isManager ? 'manager'
      : SPECIALIST_IDS.has(agId) ? 'specialist'
      : 'employee';

    // Manager gets a unique anchor position for visual consistency
    const effectiveZone = isManager
      ? { cx: 20, cy: 14 }
      : zone;

    // Find live session for this agent (strict mapping first, parsed fallback second)
    const session = findSessionForAgent(sessionsList, agId, isManager, managerAgentId);
    const tokens = session?.contextTokens ?? 0;

    // Display names come from agent identity/name; IDs remain stable in metadata
    const buildingName = isManager ? `${displayName} Command`
      : hierarchyRole === 'specialist' ? `${displayName} Lab`
      : `${displayName} Reef`;
    const labelBase = sanitizeLabel(displayName);
    const buildingLabel = isManager ? `${labelBase} COMMAND`
      : hierarchyRole === 'specialist' ? `${labelBase} LAB`
      : `${labelBase} REEF`;
    ...
    buildings.push({
      ...
      kind: 'agent',
      agentId: agId,
      hierarchyRole,          // 'manager' | 'specialist' | 'employee'
      name: buildingName,
      label: buildingLabel,
      ...placeAt(effectiveZone.cx, effectiveZone.cy, size.w, size.h),
      accent: color.css,
      wall: darken(color.hue, color.sat),
      roof: darker(color.hue, color.sat),
      // Manager has a crown marker, specialist has tools marker
      marker: isManager ? 'crown' : hierarchyRole === 'specialist' ? 'tools' : null,
      details: {
        model, tier, agentId: agId,
        displayName,
        role: hierarchyRole,
        tokens: `${Math.round(tokens/1000)}k`,
      },
      activities: isManager
        ? ['Delegating…','Reviewing…','Planning…','Supervising…']
        : hierarchyRole === 'specialist'
        ? ['Running tool…','Processing…','Returning result…','Awaiting task…']
        : ['Awaiting task…','Executing…','Reporting back…','Idle…'],
    });

    agents.push({
      id: agId,
      name: displayName,
      role: hierarchyRole === 'manager' ? 'Manager'
          : hierarchyRole === 'specialist' ? 'Specialist'
          : 'Employee',
      hierarchyRole,
      ...
      model,
      tier,
      startX: effectiveZone.cx,
      startY: effectiveZone.cy + size.h + 2,
...
// ── Persistence ───────────────────────────────────────────────────────────────
export function loadSavedLayout(clawcoveDir) {
  const p = join(clawcoveDir, 'layout.json');
  if (!existsSync(p)) return null;
  try { return JSON.parse(readFileSync(p, 'utf8')); }
  catch { return null; }
}

export function saveLayout(clawcoveDir, layout) {
  try {
    mkdirSync(clawcoveDir, { recursive: true });
    writeFileSync(join(clawcoveDir, 'layout.json'), JSON.stringify(layout, null, 2));
  } catch (e) {
    console.warn('[clawcove] could not save layout:', e.message);
  }
}
...
// ── Helpers ───────────────────────────────────────────────────────────────────
function normalizeAgentDefs(agentsCfg) {
  if (Array.isArray(agentsCfg?.list)) return agentsCfg.list.filter(a => a && a.id);
  if (Array.isArray(agentsCfg)) return agentsCfg.filter(a => a && a.id);
  return [{ id: 'main', default: true }];
}

function detectManagerAgentId(agentDefs) {
  if (!agentDefs?.length) return 'main';
  const explicitDefault = agentDefs.find(a => a?.default === true || a?.isDefault === true);
  return explicitDefault?.id ?? agentDefs[0]?.id ?? 'main';
}

function normalizeModelRef(model) {
  if (!model) return 'openai-codex/gpt-5.3-codex';
  if (typeof model === 'string') return model;
  if (typeof model === 'object' && model?.primary) return model.primary;
  return String(model);
}

function resolveAgentDisplayName(agDef, agId) {
  const n = agDef?.identity?.name ?? agDef?.name ?? agId;
  return String(n || agId).trim() || agId;
}
...
function findSessionForAgent(sessionsList, agentId, isManager, managerAgentId) {
  const byAgentField = sessionsList.find(s => s?.agentId === agentId);
  if (byAgentField) return byAgentField;

  const byParsedKey = sessionsList.find(s => parseAgentIdFromSessionKey(s?.sessionKey) === agentId);
  if (byParsedKey) return byParsedKey;

  if (isManager) {
    return sessionsList.find(s => {
      const parsed = parseAgentIdFromSessionKey(s?.sessionKey);
      return parsed === managerAgentId || s?.sessionKey?.endsWith(':main');
    }) ?? null;
  }
  return null;
}
