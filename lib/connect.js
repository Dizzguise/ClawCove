...
function toLowerAscii(s) {
  ...
  return s.trim().replace(/[A-Z]/g, (c) => String.fromCharCode(c.charCodeAt(0) + 32));
}

function buildDeviceAuthPayloadV3(device, token, nonce, signedAt) {
  const scopes = SCOPES.join(',');
  const platform = toLowerAscii('node');
  const deviceFamily = toLowerAscii('headless');
  return [
...
function signDevicePayload(device, token, nonce, signedAt) {
  const payload = buildDeviceAuthPayloadV3(device, token, nonce, signedAt);
...
function buildConnectFrame(id, nonce, signedAt, device, token) {
  const signature = signDevicePayload(device, token, nonce, signedAt);
  return {
    type:   'req',
    id,
    method: 'connect',
