...
//
//  SIGNATURE (v2 pipe-delimited payload, Ed25519-signed):
//    v2|deviceId|clientId|clientMode|role|scopesCsv|signedAtMs|tokenOrEmpty|nonce
//    Gateway verifies signature against device.publicKey — must use Ed25519, not HMAC.
//
//  DEVICE (required fields):
//    id (fingerprint derived from publicKey), publicKey (base64 Ed25519),
//    signature (hex), signedAt (ms), nonce
...
// ── Device fingerprint: gateway expects device.id to match publicKey-derived id ─
function fingerprintFromPublicKey(publicKeyDerBase64) {
  const buf = Buffer.from(publicKeyDerBase64, 'base64');
  return `clawcove-${createHash('sha256').update(buf).digest('hex').slice(0, 16)}`;
}
...
function loadOrCreateDevice(stateDir) {
  ...
  try {
    if (existsSync(p)) {
      ...
      if (d.deviceId && d.publicKeyBase64) {
        // Migrate: ensure deviceId matches publicKey fingerprint (gateway requirement)
        const fp = fingerprintFromPublicKey(d.publicKeyBase64);
        if (d.deviceId !== fp) {
          d.deviceId = fp;
  ...
  // Generate Ed25519 keypair — gateway verifies signature against publicKey
  ...
  const privB64 = privateKey.export({ type: 'pkcs8', format: 'der' })
                             .toString('base64');
  const d = {
    deviceId:       fingerprintFromPublicKey(pubB64),
...
// ── v2 signature (Ed25519) ────────────────────────────────────────────────────
// Pipe-delimited payload: v2|deviceId|clientId|clientMode|role|scopesCsv|signedAtMs|token|nonce
// Gateway verifies signature against device.publicKey — Ed25519 required.
// HMAC cannot validate against Ed25519 public key (causes INVALID_REQUEST / identity mismatch).
function signV2Ed25519(device, token, nonce, signedAt) {
  const scopesCsv = SCOPES.join(',');
  const payload   = [
    'v2', device.deviceId, CLIENT_ID, CLIENT_MODE, ROLE,
...
function buildConnectFrame(id, nonce, signedAt, device, token) {
  const signature = signV2Ed25519(device, token, nonce, signedAt);
  return {
    type:   'req',
    id,
    method: 'connect',
...
export function createGatewayClient(gatewayUrl, token, onState, onEvent, onError, opts = {}) {
  ...
  function connect() {
    ...
    function doSendConnect(nonce) {
      ...
      const frame = buildConnectFrame('1', nonce, Date.now(), device, effectiveToken);
